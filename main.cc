...
static void carbonFatalDialog(const char *error, const char *exposition) {
  ...
  if (exposition != NULL) {
    cfExposition =
        CFStringCreateWithCString(NULL, exposition, kCFStringEncodingASCII);
  } else {
    cfExposition = NULL;
  }
  ...
  // Display Dialog
  CreateStandardAlert(kAlertStopAlert, cfError, cfExposition, /* can be NULL */
                      &alertParam,                            /* can be NULL */
                      &alertDialog);
  RunStandardAlert(alertDialog, NULL, &index);
  // Cleanup
  CFRelease(cfError);
  if (cfExposition != NULL) {
...
#endif
int split_string_into_argv(char *string, int *argc_out, char **argv,
                           int max_argv) {
...
#endif /* if defined(__WXMSW__) || ((BX_WITH_SDL || BX_WITH_SDL2) &&           \
          defined(WIN32)) */
...
void print_usage(void) {
  fprintf(
      ...
      "  -benchmark N     run Bochs in benchmark mode for N millions of "
      "emulated ticks\n"
#if BX_ENABLE_STATISTICS
      ...
      "bochsrc section in the user documentation or the man page of "
      "bochsrc.\n");
#else
...
int bx_init_main(int argc, char *argv[]) {
  ...
  while (arg < argc) {
    ...
    if (!strcmp("--help", argv[arg]) || !strncmp("-h", argv[arg], 2)
      ...
      if (++arg >= argc)
        BX_PANIC(("-log must be followed by a filename"));
      else
        SIM->get_param_string(BXPN_LOG_FILENAME)->set(argv[arg]);
      ...
      if (++arg >= argc)
        BX_PANIC(("-dbglog must be followed by a filename"));
      else
        SIM->get_param_string(BXPN_DEBUGGER_LOG_FILENAME)->set(argv[arg]);
    }
#endif
      ...
      if (++arg >= argc)
        ...
        SIM->get_param_num(BXPN_BOCHS_BENCHMARK)->set(atoi(argv[arg]));
    }
#if BX_ENABLE_STATISTICS
    else if (!strcmp("-dumpstats", argv[arg])) {
      if (++arg >= argc)
        BX_PANIC(("-dumpstats must be followed by a number"));
      else
        SIM->get_param_num(BXPN_DUMP_STATS)->set(atoi(argv[arg]));
    }
#endif
  ...
  if (!getenv("BXSHARE")) {
    ...
    if (!CFURLGetFileSystemRepresentation(
            bxshareDir, true, (UInt8 *)bxshareDirPath, MAXPATHLEN)) {
      BX_PANIC(
          ("Unable to work out bxshare path! (Most likely path too long!)"));
      return -1;
    }
    char *c;
  ...
  // also if there is no LTDL_LIBRARY_PATH, but we have a bundle since we're
  // here This is here so that it is available whenever --with-carbon is defined
  // but the above code might be skipped, as in --with-sdl --with-carbon
  if (!isatty(STDIN_FILENO) || !getenv("LTDL_LIBRARY_PATH")) {
    ...
    // our libs have been placed in bochs.app/Contents/(current platform aka
    // MacOS)/lib This isn't quite right, but they are platform specific and we
    // haven't put our plugins into true frameworks and bundles either
    mainBundle = CFBundleGetMainBundle();
    BX_ASSERT(mainBundle != NULL);
    libDir = CFBundleCopyAuxiliaryExecutableURL(mainBundle, CFSTR("lib"));
    BX_ASSERT(libDir != NULL);
    // translate this to a unix style full path
    if (!CFURLGetFileSystemRepresentation(libDir, true, (UInt8 *)libDirPath,
                                          MAXPATHLEN)) {
      BX_PANIC(("Unable to work out ltdl library path within bochs bundle! "
                "(Most likely path too long!)"));
      return -1;
    }
...
#else
    BX_INFO(
        ("BXSHARE not set. using compile time default '%s'", BX_SHARE_PATH));
...
#endif
    if (bochsrc_filename == NULL)
      bochsrc_filename = bx_find_bochsrc();
    if (bochsrc_filename)
...
bx_bool load_and_init_display_lib(void) {
  ...
  if (!strcmp(gui_name, "wx")) {
    ...
    BX_ERROR(("wxWidgets was not used as the configuration interface, so it "
              "cannot be used as the display library"));
...
int bx_begin_simulation(int argc, char *argv[]) {
...
#if BX_SUPPORT_APIC
  simulate_xapic =
      (SIM->get_param_enum(BXPN_CPUID_APIC)->get() >= BX_CPUID_SUPPORT_XAPIC);
  ...
  if (bx_cpu_count > max_smp_threads) {
    BX_PANIC(("cpu: too many SMP threads defined, only %u threads supported by "
              "%sAPIC",
  ...
  SIM->get_param_bool(BXPN_MOUSE_ENABLED)
      ->set(SIM->get_param_bool(BXPN_MOUSE_ENABLED)->get());
...
void bx_init_hardware() {
  ...
  if (SIM->get_param_string(BXPN_LOG_FILENAME)->getptr()[0] != '-') {
    BX_INFO(("using log file %s",
             SIM->get_param_string(BXPN_LOG_FILENAME)->getptr()));
  ...
  const char *msrs_file =
      SIM->get_param_string(BXPN_CONFIGURABLE_MSRS_PATH)->getptr();
  ...
  if (!cpu_model) {
    ...
    BX_INFO(("  SIMD support: %s",
             SIM->get_param_enum(BXPN_CPUID_SIMD)->get_selected()));
    ...
    BX_INFO(("  XSAVE support: %s %s", xsave_enabled ? "xsave" : "no",
             xsaveopt_enabled ? "xsaveopt" : ""));
  ...
  BX_INFO(("  RepeatSpeedups support: %s",
           BX_SUPPORT_REPEAT_SPEEDUPS ? "yes" : "no"));
  BX_INFO(("  Fast function calls: %s", BX_FAST_FUNC_CALL ? "yes" : "no"));
  BX_INFO(("  Handlers Chaining speedups: %s",
           BX_SUPPORT_HANDLERS_CHAINING_SPEEDUPS ? "yes" : "no"));
  BX_INFO(("Devices configuration"));
  BX_INFO(
      ("  PCI support: %s", BX_SUPPORT_PCI ? "i440FX i430FX i440BX" : "no"));
#if BX_SUPPORT_NE2K || BX_SUPPORT_E1000
  BX_INFO(("  Networking support:%s%s", BX_SUPPORT_NE2K ? " NE2000" : "",
           BX_SUPPORT_E1000 ? " E1000" : ""));
#else
  BX_INFO(("  Networking: no"));
#endif
#if BX_SUPPORT_SB16 || BX_SUPPORT_ES1370
  BX_INFO(("  Sound support:%s%s", BX_SUPPORT_SB16 ? " SB16" : "",
           BX_SUPPORT_ES1370 ? " ES1370" : ""));
#else
  BX_INFO(("  Sound support: no"));
#endif
#if BX_SUPPORT_PCIUSB
  BX_INFO(("  USB support:%s%s%s%s", BX_SUPPORT_USB_UHCI ? " UHCI" : "",
           BX_SUPPORT_USB_OHCI ? " OHCI" : "",
           BX_SUPPORT_USB_EHCI ? " EHCI" : "",
           BX_SUPPORT_USB_XHCI ? " xHCI" : ""));
#else
  ...
  if (benchmark_mode) {
    BX_INFO(
        ("Bochs benchmark mode is ON (~%d millions of ticks)", benchmark_mode));
    bx_pc_system.register_timer_ticks(&bx_pc_system,
                                      bx_pc_system_c::benchmarkTimer,
                                      (Bit64u)benchmark_mode * 1000000,
                                      0 /* one shot */, 1, "benchmark.timer");
  }
  ...
  if (dumpstats) {
    ...
    bx_pc_system.register_timer_ticks(
        &bx_pc_system, bx_pc_system_c::dumpStatsTimer,
  ...
  if (ips_count) {
    ...
    if (bx_dbg.print_timestamps) {
      printf("IPS: %u\taverage = %u\t\t(%us)\n", (unsigned)ips_count,
             (unsigned)(ticks_count / counts), (unsigned)counts);
      fflush(stdout);
    }
  }
  return;
...
void CDECL bx_signal_handler(int signum) {
  ...
  if (!SIM->is_sim_thread()) {
    BX_INFO(("bx_signal_handler: ignored sig %d because it wasn't called from "
...
#endif
  // Exit immediately when ctrl-c is pressed without prompting
  if (signum == SIGINT) {
    exit(0);
  }
