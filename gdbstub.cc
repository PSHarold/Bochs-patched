/////////////////////////////////////////////////////////////////////////
// $Id: gdbstub.cc 14019 2020-12-08 19:52:39Z vruppert $
/////////////////////////////////////////////////////////////////////////
//
...
static Bit64u breakpoints[MAX_BREAKPOINTS] = {
    0,
};
...
void bx_gdbstub_break(void) { bx_enter_gdbstub = 1; }
...
static int access_linear(Bit64u laddress, unsigned len, unsigned int rw,
                         Bit8u *data) {
  bx_phy_address phys;
  bx_bool valid;
...
static void debug_loop(void) {
  ...
  while (ne == 0) {
    ...
    // Stubs that only control single-threaded targets can implement run control
    // with the ‘c’ (continue), and ‘s’ (step) commands. Stubs that support
...
#define PUTREG(buf, val, len)                                                  \
  do {                                                                         \
    ...
    // et.al.). op depends on the operation to be performed: it should be ‘c’
    // for step and continue operations (note that this is deprecated,
    ...
    // ‘q name params...’
    // ‘Q name params...’
    // General query (‘q’) and set (‘Q’). These packets are described fully
    // in Section E.4 [General Query Packets]
    case 'q':
      if (buffer[1] == 'C') {
    ...
    // ‘z type,addr,kind’
    // ‘Z type,addr,kind’
    // Insert (‘Z’) or remove (‘z’) a type breakpoint or watchpoint starting
    // at address address of kind kind.
    case 'Z':
...
static void wait_for_connect(int portn) {
  ...
  opt = 1;
#if defined(__MINGW32__) || defined(_MSC_VER)
  r = setsockopt(listen_socket_fd, SOL_SOCKET, SO_REUSEADDR, (const char *)&opt,
                 sizeof(opt));
#else
  ...
  socket_fd =
      accept(listen_socket_fd, (struct sockaddr *)&sockaddr, &sockaddr_len);
  if (socket_fd == -1) {
  ...
  opt = 1;
#if defined(__MINGW32__) || defined(_MSC_VER)
  r = setsockopt(socket_fd, protoent->p_proto, TCP_NODELAY, (const char *)&opt,
                 sizeof(opt));
#else
